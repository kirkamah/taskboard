...
// Read locale from the browser cookie. Used by client-only pages (e.g. the
// auth flow) that don't have a server wrapper passing a prop down.
export function getClientLocale() {
  if (typeof document === 'undefined') return 'ru';
  const m = document.cookie.match(/(?:^|;\s*)tb_locale=([^;]+)/);
