...
export default function SignupPage() {
  ...
  const handleSignup = async (e) => {
    ...
    if (data.user && !data.session) {
      setError(t('auth.signup.confirmEmail'));
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6">
      <div className="w-full max-w-md bg-white border border-gray-200 rounded-lg p-8">
        <h1 className="text-2xl font-semibold text-gray-900 mb-1">{t('auth.signup.title')}</h1>
        <p className="text-sm text-gray-500 mb-6">{t('auth.signup.subtitle')}</p>

        <form onSubmit={handleSignup} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-gray-700 uppercase tracking-wide mb-2">{t('auth.signup.name')}</label>
            <input
              type="text"
              value={name}
        ...
        >
          <GoogleIcon /> {t('auth.signup.googleButton')}
        </button>
