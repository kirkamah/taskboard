...
export default async function TeamsPage() {
  ...
  // Подсчёт участников и задач для каждой комнаты
  const enriched = await Promise.all(rooms.map(async (room) => {
    ...
    return { ...room, memberCount: memberCount || 0, taskCount: taskCount || 0 };
  }));
            ...
            {enriched.map(room => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs px-2 py-1 border border-gray-300 rounded">{roleLabel[room.role]}</span>
                </div>
              </Link>
            ))}
          </div>
