...
export default function PomodoroTimer({ taskId, initialSpentMinutes = 0, canEdit = true, locale = 'ru' }) {
  ...
  const fmtSpent = () => {
    const minLbl = t('pomodoro.minShort');
    const hourLbl = t('pomodoro.hourShort');
    if (spent < 60) return `${spent} ${minLbl}`;
    const h = Math.floor(spent / 60);
    const m = spent % 60;
    return `${h} ${hourLbl} ${m ? `${m} ${minLbl}` : ''}`.trim();
  };

  return (
    <div className="mt-4 pt-4 border-t border-gray-100">
      <p className="text-xs font-medium text-gray-700 uppercase tracking-wide mb-2 flex items-center gap-2">
        <Timer size={12} /> {t('pomodoro.heading')}
      </p>
        ...
        {isOurs ? (
          <>
            <span className={`px-2 py-1 rounded text-sm font-mono ${active.phase === 'focus' ? 'bg-gray-900 text-white' : 'bg-green-100 text-green-800'}`}>
              {active.phase === 'focus' ? t('pomodoro.focus') : t('pomodoro.break')} · {fmt(remainingMs)}
            </span>
            <button onClick={stop} className="px-2.5 py-1 text-xs border border-gray-300 rounded-md bg-white hover:bg-gray-50 inline-flex items-center gap-1">
              <Square size={12} /> {t('pomodoro.stop')}
            </button>
          </>
        ) : otherTaskRunning ? (
          <span className="text-xs text-gray-500">{t('pomodoro.otherTask')}</span>
        ) : (
          canEdit && (
            <button onClick={start} className="px-2.5 py-1 text-xs border border-gray-300 rounded-md bg-white hover:bg-gray-50 inline-flex items-center gap-1">
              <Play size={12} /> {t('pomodoro.start')}
            </button>
          )
        )}
        <span className="text-xs text-gray-500">{t('pomodoro.spent')}: <strong className="text-gray-800">{fmtSpent()}</strong></span>
      </div>
    </div>
  );
}
