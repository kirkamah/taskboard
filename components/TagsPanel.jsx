...
export default function TagsPanel({ roomId, ownerId, tags, locale = 'ru' }) {
  const t = (k, p) => translate(locale, k, p);
  // Either roomId (room) or ownerId (personal board) must be set.
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(false);
    if (error) {
      alert(t('tags.deleteFailed', { message: error.message }));
      return;
    }
  ...
  const err2msg = (e) => {
    if (e?.code === '23505') return t('tags.duplicate');
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-4 h-fit">
      <div className="flex items-center justify-between mb-3">
        <h2 className="font-semibold text-gray-900 flex items-center gap-2">
          <TagIcon size={16} /> {t('tags.heading')} ({tags.length})
        </h2>
        <button
          onClick={openCreate}
          className="text-xs px-2 py-1 bg-gray-900 text-white rounded hover:bg-gray-800 flex items-center gap-1"
        >
          <Plus size={12} /> {t('tags.new')}
        </button>
      </div>

      {tags.length === 0 ? (
        <p className="text-xs text-gray-400">{t('tags.empty')}</p>
      ) : (
        <div className="space-y-2">
          {tags.map((tag) => (
            <div key={tag.id} className="flex items-center justify-between gap-2 border border-gray-200 rounded-md p-2">
              <Tag tag={tag} />
      ...
      {showEditor && (
        <Modal onClose={() => setShowEditor(false)}>
          <div className="flex items-center justify-between p-6 border-b border-gray-200">
            <h2 className="text-lg font-semibold">{editingTag ? t('tags.editTitle') : t('tags.newTitle')}</h2>
          ...
          <div className="p-6 space-y-4">
            <div>
              <label className="block text-xs font-medium text-gray-700 uppercase tracking-wide mb-2">{t('tags.fieldName')}</label>
              <input
                type="text"
                value={formName}
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 uppercase tracking-wide mb-2">{t('tags.color')}</label>
              <ColorPicker value={formColor} onChange={setFormColor} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700 uppercase tracking-wide mb-2">{t('tags.preview')}</label>
              <Tag tag={{ name: formName.trim() || t('tags.previewName'), color: formColor }} />
            </div>
          ...
          <div className="flex justify-end gap-2 p-4 border-t border-gray-200 bg-gray-50 rounded-b-lg">
            ...
            >
              <Check size={14} /> {saving ? t('tags.saving') : t('common.save')}
            </button>
          </div>
        </Modal>
      )}
      ...
      {deleteConfirm && (
        <Modal onClose={() => setDeleteConfirm(null)}>
          <div className="flex items-center justify-between p-6 border-b border-gray-200">
            <h2 className="text-lg font-semibold text-red-700">{t('tags.deleteTitle')}</h2>
          ...
          <div className="p-6 space-y-3">
            <p className="text-sm text-gray-700">
              <Tag tag={deleteConfirm} /> {t('tags.deleteBody.suffix')}
            </p>
          ...
          <div className="flex justify-end gap-2 p-4 border-t border-gray-200 bg-gray-50 rounded-b-lg">
            ...
            >
              {deleting ? t('tags.deleting') : t('common.delete')}
            </button>
          </div>
        </Modal>
      )}
