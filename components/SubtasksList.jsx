...
export default function SubtasksList({ parent, userId, onOpen, canEdit = true, locale = 'ru' }) {
  ...
  return (
    <div className="mt-4 pt-4 border-t border-gray-100">
      <p className="text-xs font-medium text-gray-700 uppercase tracking-wide mb-2 flex items-center gap-2">
        <ListTree size={12} /> {t('subtasks.heading')}{total > 0 ? ` · ${done}/${total}` : ''}
      </p>
      ...
      {canEdit && (
        <div className="flex items-center gap-2">
          ...
          >
            <Plus size={12} /> {adding ? '…' : t('subtasks.add')}
          </button>
        </div>
      )}
    </div>
